...
class GasStation{
    ...
    // 차량에 넣을 연료량을 리턴
    // 따로 질문을 받는 방법은 추후에 추가
    fueling(fuel = 20){
        if(gas == 0){
            console.log("주유할 연료가 없습니다.");
            return 0;
        }
        this.gas -= fuel;
        console.log(`주유소에서 ${fuel}L 만큼의 연료를 가져왔습니다`);
        return fuel;
    }

    // 연료가 없는 차량을 등록
    carStock(car){
        this.emptyCar.push(car);
        console.log(`[${car.id}]을(를) 주유소에 입고했습니다`)
    }
    
    // 연료를 충전 후 차량을 보냄
    carRelease(){
        emptyCar[0].fueling(this.fueling);
        console.log(`[${emptyCar[0].id}]을(를) 주유소에 출고했습니다`)
        return emptyCar.shift();
    }
}
...
function autoDriveCars(){
    ...
    let car = cars.shift(); 
    // 꺼낸 car를 driving 시킨다.
    // 다만 주행 후 거리가 0일 경우 연료가 없다고 판단하여, emptyCar에 넣는다
     if(car.drive(rangeRandom(5, 30)) == 0){
        gasStation.carStock(car);
     } else {
        // driving 끝난 차는 cars 맨 뒤에 넣는다.
        cars.push(car);
     }

     if(gasStation.emptyCar.length != 0){
        cars.push(gasStation.carRelease());
     }
}
